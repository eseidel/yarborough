...
impl App {
    ...
    fn run_test(&mut self, s_idx: usize, t_idx: usize) {
        ...
        // Hand
        let hand = bridge_core::io::hand_parser::parse_hand(&test.hand);

        // Vulnerability
        let _vulnerability = match test.vulnerability.as_str() {
            "N-S" | "NS" => bridge_core::board::Vulnerability::NS,
            "E-W" | "EW" => bridge_core::board::Vulnerability::EW,
            "Both" | "All" => bridge_core::board::Vulnerability::Both,
            _ => bridge_core::board::Vulnerability::None,
        };

        // History
        let history = bidding_utils::parse_calls(&test.history);
        let mut auction = bridge_core::auction::Auction::new(bridge_core::board::Position::North);
        for call in &history {
        ...
        // Trace
        let trace = bridge_engine::nbk::select_bid_with_trace(&hand, &auction, current_player);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Note: If running from cargo run --bin associated with bridge-engine crate, CWD might be the user workspace root.
