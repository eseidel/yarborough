...
class VulnerabilityTest(unittest.TestCase):
    def test_vunerability_from_board_number(self):
        ...
        for number, expected_vulnerability in list(expectations.items()):
            self.assertEqual(Vulnerability.from_board_number(
                number).name, expected_vulnerability)
...
class CallHistoryTest(unittest.TestCase):
    ...
    def test_position_to_call(self):
        self.assertEqual(CallHistory.from_string("").position_to_call(), NORTH)
        self.assertEqual(CallHistory.from_string("P").position_to_call(), EAST)
        self.assertEqual(CallHistory.from_string(
            "1N P").position_to_call(), SOUTH)
        self.assertEqual(CallHistory.from_string(
            "1N P 2C").position_to_call(), WEST)
        self.assertEqual(CallHistory.from_string(
            "1N P 2C P").position_to_call(), NORTH)
        self.assertEqual(CallHistory.from_string(
            "1N P 2C P 2D").position_to_call(), EAST)

    def test_last_to_call(self):
        self.assertEqual(CallHistory.from_string("").last_to_call, None)
        self.assertEqual(CallHistory.from_string("P").last_to_call, NORTH)
        self.assertEqual(CallHistory.from_string("1N P").last_to_call, EAST)
        self.assertEqual(CallHistory.from_string(
            "1N P 2C").last_to_call, SOUTH)
        self.assertEqual(CallHistory.from_string(
            "1N P 2C P").last_to_call, WEST)
        self.assertEqual(CallHistory.from_string(
            "1N P 2C P 2D").last_to_call, NORTH)

    def test_is_passout(self):
        self.assertEqual(CallHistory.from_string("").is_passout(), False)
        self.assertEqual(CallHistory.from_string("P").is_passout(), False)
        self.assertEqual(CallHistory.from_string("P P").is_passout(), False)
        self.assertEqual(CallHistory.from_string("P P P").is_passout(), False)
        self.assertEqual(CallHistory.from_string("P P P P").is_passout(), True)
        self.assertEqual(CallHistory.from_string(
            "P 1N P P P").is_passout(), False)

    def test_copy_with_partial_history(self):
        history = CallHistory.from_string("P P 1N P P P")
        self.assertEqual(len(history.calls), 6)
        self.assertEqual(len(history.copy_with_partial_history(0).calls), 0)
        self.assertEqual(len(history.copy_with_partial_history(2).calls), 2)
        self.assertEqual(len(history.copy_with_partial_history(-2).calls), 4)
        partial_history = history.copy_with_partial_history(3)
        self.assertEqual(len(history.calls), 6)
        self.assertEqual(len(partial_history.calls), 3)
        partial_history.calls.append(None)  # Invalid, but fine for testing.
        self.assertEqual(len(history.calls), 6)
        self.assertEqual(len(partial_history.calls), 4)

    def _assert_competative_auction(self, history_string, is_competative):
        self.assertEqual(CallHistory.from_string(
            history_string).competative_auction(), is_competative)
    ...
    def _assert_opener(self, history_string, dealer, opener):
        self.assertEqual(CallHistory.from_string(
            history_string, dealer.char).opener(), opener)
    ...
    def test_from_identifier(self):
        self.assertEqual(CallHistory.from_identifier(
            'E:EW:P').identifier, CallHistory.from_string('P', 'E', 'E-W').identifier)
        # Test that from_identifier is forgiving of a missing trailing colon
        self.assertEqual(CallHistory.from_identifier(
            'E:EW:').identifier, CallHistory.from_string('', 'E', 'E-W').identifier)
        self.assertEqual(CallHistory.from_identifier(
            'E:EW').identifier, CallHistory.from_string('', 'E', 'E-W').identifier)
        # Test that from_identifier is forgiving of a trailing comma.
        self.assertEqual(CallHistory.from_identifier(
            'N:NO:P,').calls_string(), "P")
    ...
    def _assert_is_legal_call(self, history_string, call_name, is_legal=True):
        ...
        self.assertEqual(call_history.is_legal_call(call), is_legal)
    ...
    def test_empty_for_board_number(self):
        self.assertEqual(CallHistory.empty_for_board_number(1).dealer, NORTH)
        self.assertEqual(CallHistory.empty_for_board_number(6).dealer, EAST)
        self.assertEqual(CallHistory.empty_for_board_number(16).dealer, WEST)
